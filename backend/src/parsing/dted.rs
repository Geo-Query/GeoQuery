...
fn parse_dddmmssh(data: &[u8]) -> Result<f64, DT2ErrorState> {
    if data.len() != 8 {
        return Err(DT2ErrorState::DSIError(DSIErrorState::InvalidLength(
            data.len(),
        )));
    ...
    let degrees = String::from_utf8(data[0..3].to_vec())
        .map_err(|_| {
            DT2ErrorState::DSIError(DSIErrorState::InvalidDDDMMSSH(data.try_into().unwrap()))
        })?
        .parse::<f64>()
        .map_err(|_| {
            DT2ErrorState::DSIError(DSIErrorState::InvalidDDDMMSSH(data.try_into().unwrap()))
    ...
    if hemisphere != 'N' && hemisphere != 'S' && hemisphere != 'W' && hemisphere != 'E' {
        return Err(DT2ErrorState::DSIError(DSIErrorState::InvalidDDDMMSSH(
...
fn parse_ddmmssh(data: &[u8]) -> Result<f64, DT2ErrorState> {
    ...
    let degrees = String::from_utf8(data[0..2].to_vec())
        .map_err(|_| {
            DT2ErrorState::DSIError(DSIErrorState::InvalidDDMMSSH(data.try_into().unwrap()))
        })?
        .parse::<f64>()
        .map_err(|_| {
            DT2ErrorState::DSIError(DSIErrorState::InvalidDDMMSSH(data.try_into().unwrap()))
    ...
    if hemisphere != 'N' && hemisphere != 'S' && hemisphere != 'W' && hemisphere != 'E' {
        return Err(DT2ErrorState::DSIError(DSIErrorState::InvalidDDMMSSH(
            data.try_into().unwrap(),
        )));
    }
...
impl UserHeaderLabel {
    pub fn from_bytes(buffer: &[u8]) -> Result<UserHeaderLabel, DT2ErrorState> {
        if buffer.len() != 80 {
            return Err(DT2ErrorState::UHLError(UHLErrorState::InvalidLength(
                buffer.len(),
            )));
        }
        let sentinel = &buffer[0..4];
        if sentinel != [85, 72, 76, 49] {
            // Assert equivalent to given byte string (UHL1 in ASCII)
            return Err(DT2ErrorState::UHLError(UHLErrorState::InvalidSentinel(
                <[u8; 4]>::try_from(sentinel).unwrap(),
            )));
        }
        ...
        let longitude = match parse_dddmmssh(longitude_str) {
            ...
            Err(e) => {
                ...
                return Err(DT2ErrorState::UHLError(UHLErrorState::InvalidDDMMSSH(
                    longitude_str.try_into().unwrap(),
                )));
            }
        };
...
impl DataSetIdentification {
    pub fn from_bytes(buffer: &[u8]) -> Result<DataSetIdentification, DT2ErrorState> {
        ...
        if sentinel != [68, 83, 73, 85] {
            return Err(DT2ErrorState::DSIError(DSIErrorState::InvalidSentinel(
                sentinel.try_into().unwrap(),
            )));
        }
...
pub fn parse_dted(reader: &mut BufReader<File>) -> Result<DT2MetaData, DT2ErrorState> {
    ...
    let _uhl = match reader.read_exact(&mut uhl_buf) {
        ...
        Err(e) => {
            return Err(DT2ErrorState::UnexpectedFormat(
                format!("Failed to read bytes: {e:?}").to_string(),
            ));
        }
    };
...
mod tests {
    ...
    fn test_parse_dddmmssh_with_tempfile_valid() {
        ...
        temp_file
            ...
            .expect("Failed to seek tempfile");
    ...
    fn test_user_header_label_from_bytes_with_tempfile_invalid_length() {
        ...
        assert!(matches!(
            result,
            Err(DT2ErrorState::UHLError(UHLErrorState::InvalidLength(_)))
        ));
    }
}
