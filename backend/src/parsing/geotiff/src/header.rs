...
pub fn parse_header(buffer: &[u8]) -> Result<(ByteOrder, SeekFrom), TIFFErrorState> {
    if buffer.len() != 8 {
        return Err(TIFFErrorState::HeaderError(
            HeaderErrorState::InvalidLength(buffer.len()),
        ));
    }
    ...
    let byte_order = match buffer[0..2] {
        ...
        _ => {
            return Err(TIFFErrorState::HeaderError(
                HeaderErrorState::UnexpectedByteOrder([buffer[0], buffer[1]]),
            ))
        }
    };
    ...
    if !match byte_order {
        ...
        return Err(TIFFErrorState::HeaderError(
            HeaderErrorState::UnexpectedMagicNumber([magic_numbers[0], magic_numbers[1]]),
        ));
    }
    let ifd_offset = SeekFrom::Start(u32::from_bytes(&buffer[4..8], &byte_order) as u64);
...
mod tests {
    ...
    fn test_parse_header_invalid_length() {
        ...
        assert!(matches!(
            result,
            Err(TIFFErrorState::HeaderError(
                HeaderErrorState::InvalidLength(_)
            ))
        ));
    }
    ...
    fn test_parse_header_unexpected_byte_order() {
        ...
        assert!(matches!(
            result,
            Err(TIFFErrorState::HeaderError(
                HeaderErrorState::UnexpectedByteOrder(_)
            ))
        ));
    }
    ...
    fn test_parse_header_unexpected_magic_number() {
        ...
        assert!(matches!(
            result,
            Err(TIFFErrorState::HeaderError(
                HeaderErrorState::UnexpectedMagicNumber(_)
            ))
        ));
    }
    ...
    fn test_parse_header_specific_error_values() {
        ...
        let buffer = [0x00, 0x00, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00];
        if let Err(TIFFErrorState::HeaderError(HeaderErrorState::UnexpectedByteOrder(
            error_bytes,
        ))) = parse_header(&buffer)
        {
        ...
        if let Err(TIFFErrorState::HeaderError(HeaderErrorState::UnexpectedMagicNumber(
            error_bytes,
        ))) = parse_header(&buffer)
        {
    ...
    fn test_parse_header_empty_buffer() {
        ...
        assert!(matches!(
            result,
            Err(TIFFErrorState::HeaderError(
                HeaderErrorState::InvalidLength(0)
            ))
        ));
    }
    #[test]
    fn test_parse_header_buffer_too_long() {
        ...
        assert!(matches!(
            result,
            Err(TIFFErrorState::HeaderError(
                HeaderErrorState::InvalidLength(9)
            ))
        ));
    }
    #[test]
    fn test_parse_header_invalid_magic_number_with_correct_byte_order() {
        ...
        assert!(matches!(
            result,
            Err(TIFFErrorState::HeaderError(
                HeaderErrorState::UnexpectedMagicNumber([0xFF, 0xFF])
            ))
        ));
    }
    #[test]
