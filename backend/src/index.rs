...
impl Error for ParseErrorKind {
    fn description(&self) -> &str {
        match self {
            ParseErrorKind::UnparseableExtension => "Failed to convert OsStr into &str; Unexpected, should panic!"
        }
    }
}
...
pub fn parse(file_meta: Arc<FileMeta>) -> Result<Option<Node>, Box<dyn Error>> {
    ...
    match file_meta.path.extension().and_then(OsStr::to_str) {
        Some(ext) => match ext {
            "kml" => match parse_kml(&mut reader) {
                Ok(metadata) => Ok(Some(Node {
                    file: file_meta,
                    metadata: metadata.into()
                })),
                Err(e) => Err(e.into())
            },
            "tif" => match parse_tiff(&mut reader) {
                Ok(metadata) => Ok(Some(Node {
                    file: file_meta,
                    metadata: metadata.into()
                })),
            ...
            "dt1" | "dt2" => match parse_dt2(&mut reader) {
                Ok(metadata) => Ok(Some(Node{
                    file: file_meta,
                    metadata: metadata.into()
                })),
            ...
            "geojson" => match parse_geojson(&mut reader) {
                Ok(metadata) => Ok(Some(Node {
                    file: file_meta,
                    metadata: metadata.into()
                })),
